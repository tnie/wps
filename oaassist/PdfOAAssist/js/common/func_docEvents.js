...
function OnWindowActivate() {
    ...
    var l_doc = wps.Application.ActiveDocument;
...
function SetCurrDocEnvProp(doc) {
    ...
    if (l_bIsOADoc == true) {
        ...
        if (l_userName != "") {
            wps.Application.UserName = l_userName;
            return;
        }
    }
    //如果是非OA文件或者参数的值是空值，则按WPS安装默认用户名设置
    wps.Application.UserName = wps.PluginStorage.getItem("WPSInitUserName");
}
...
function pSetetAppUserName() {
    ...
    if (wps.Application.Workbooks.Count == 1) {
        var l_strUserName = wps.PluginStorage.getItem("WPSInitUserName");
        wps.Application.UserName = l_strUserName;
    }
}
