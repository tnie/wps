    ...
    if (l_strFileUrl) {
        ...
        if (pCheckIsExistOpenOADoc(l_strFileUrl) == true) {
            ...
            if (l_AllowOADocReOpen == false) {
                ...
                //处理重复打开相同OA 文件的方法
                var nDocCount = wps.Application.Documents.Count;
                pReOpenOADoc(l_strFileUrl);
                //重复打开的文档采用不落地的方式打开
                // 不落地方式打开文档判断落地比较多，V1版本先暂时关闭
                l_IsOnlineDoc = true;
                var nDocCount_New = wps.Application.Documents.Count;
                if (nDocCount_New > nDocCount) {
                    doc = wps.Application.ActiveDocument;
                }
            }
        } else {
...
function pDoOpenOADocProcess(params, TempLocalFile) {
    ...
    var l_Doc;
    l_Doc = wps.Application.Documents.Open(TempLocalFile, false, false, undefined, l_strDocPassword);
...
function OnDocSaveByAutoTimer() {
    ...
    var l_Count = 0
    var l_docCounts = wps.Application.Documents.Count;
    for (l_Count = 0; l_Count < l_docCounts; l_Count++) {
        l_Doc = wps.Application.Documents.Item(l_Count);
        if (l_Doc) {
