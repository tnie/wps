...
var pluginType = WpsInvoke.ClientType.pdf?WpsInvoke.ClientType.pdf:"pdf"//加载项类型wps,et,wpp,pdf
var pluginName = "PdfOAAssist";//加载项名称
...
function multInvoke(info,front,jsPluginsXml,isSilent){
    wpsClient.jsPluginsXml = jsPluginsXml ? jsPluginsXml : "http://127.0.0.1:3888/jsplugins.xml";
...
function openDoc() {
    var filePath = prompt("请输入打开文件路径（本地或是url）：", GetDemoPath("样章.pdf"))
...
window.onload = function () {
    ...
    for (var i = 0; i < btns.length; i++) {
        btns[i].onclick = function (event) {
            ...
            document.getElementById("codeDes").innerText = _pdf[this.id].detail.toString()
            document.getElementById("code").innerText = _pdf[this.id].code.toString()
            document.getElementById("demoBtn").onclick = _pdf[this.id].action;
